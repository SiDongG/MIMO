...
%% 
%Set the antenna spacing if not specified by input, the default
if  nargin < 4
    %Half a wavelength distance
    antennaSpacing = 1/2;
end
%Set angular distribution to Gaussian if not specified by input, the
%default
if nargin<5
...
for column = 1:M
    ...
    if strcmp(distribution,'Gaussian') %Compares distribution and gaussian, returns 1 if identical, 0 otherwise. 
